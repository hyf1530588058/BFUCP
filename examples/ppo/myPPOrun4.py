...
def run_ppo4(          #此文件功能与ppo原文件夹中的run文件功能一致#
    ...
    for j in range(num_updates):
        ...
        save_dir = os.path.join(origin_dir,"MDP&Model_catcher",str(saving_convention[1]))  
        if (j+1)%250 == 0:
        ...
        if j == 0:   #记录zero-shots#
            ...
            file_path = os.path.join(origin_dir,"MDP&Model_catcher",str(saving_convention[1]))  
            os.makedirs(file_path, exist_ok=True)
            f = os.path.join(file_path,"step_mean_reward.txt")
            with open(f, 'a') as file:
                file.write(str(j) + "\t\t" + str(determ_avg_reward0) + "\n")         
            # f = os.path.join(file_path,"step_HGT_0_att.pt")
            # torch.save(v_attention_maps,f)
        ...
        if (args.eval_interval is not None and len(episode_rewards) > 1
            ...
            # if int(j) == 1000:   
            #     a = os.path.join(file_path,"step_HGT_1000att.pt")
            #     torch.save(v_attention_maps,a)   
            ...
            if determ_avg_reward > max_determ_avg_reward:
                ...
                # b = os.path.join(file_path,"step_HGT_best_att.pt")    
                # torch.save(v_attention_maps,b)
