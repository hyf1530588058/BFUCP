...
def run_advmetappo_2(          #此文件功能与ppo原文件夹中的run文件功能一致#
    ...
    determ_avg_reward0 = evaluate(args.num_evals, pro_actor_critic, obs_rms, args.env_name, structure,args.seed,
            args.num_processes, eval_log_dir, device)
    file_path = os.path.join(origin_dir,"RARL_MLP_onlyact_walker_noisytest_0.5")  
    os.makedirs(file_path, exist_ok=True)
    f = os.path.join(file_path,"step_mean_reward.txt")
    with open(f, 'a') as file:
        file.write(str(saving_convention[1]) + "\t\t" + str(determ_avg_reward0) + "\n") 

    return determ_avg_reward0
    ...
    #         determ_avg_reward0 = evaluate(args.num_evals, pro_actor_critic, obs_rms, args.env_name, structure,args.seed,
    #                 args.num_processes, eval_log_dir, device)
    #         file_path = os.path.join(origin_dir,"RARL_MLP_onlyact_walker",str(saving_convention[1]))  
    #         os.makedirs(file_path, exist_ok=True)
    #         f = os.path.join(file_path,"step_mean_reward.txt")
    #         with open(f, 'a') as file:
