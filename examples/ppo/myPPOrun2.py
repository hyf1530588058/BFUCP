...
def run_ppo(          #此文件功能与ppo原文件夹中的run文件功能一致#
    ...
    for j in range(num_updates):
        ...
        if j == 0:   #记录zero-shots#
            ...
            if determ_avg_reward0 > max_determ_avg_reward:
                max_determ_avg_reward = determ_avg_reward0
            file_path = os.path.join(origin_dir,"test_HM",str(saving_convention[1]))  
            os.makedirs(file_path, exist_ok=True)
            f = os.path.join(file_path,"step_mean_reward.txt")
            with open(f, 'a') as file:
                file.write(str(j) + "\t\t" + str(determ_avg_reward0) + "\n")  

        drive_mask = (structure[0].flatten() == 3) | (structure[0].flatten() == 4)
        drive_indices = torch.from_numpy(np.flatnonzero(drive_mask)).to(device)
        ...
        for step in range(args.num_steps):
            ...
            with torch.no_grad():
                ...
                #     rollouts.masks[step])
                value, action, action_log_prob, recurrent_hidden_states,v_attention_maps = actor_critic.act(     #动作#
                    structure,rollouts.obs[step])
            # attn_maps = torch.tensor([item.cpu().detach().numpy() for item in v_attention_maps]).numpy()
            # Obser reward and next obs
            masked_action = action[:, drive_indices]

            obs, reward, done, infos = envs.step(masked_action)
            # track rewards
            for info in infos:
