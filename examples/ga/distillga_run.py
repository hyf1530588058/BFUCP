...
def run_ga(experiment_name, load_name,structure_shape, pop_size, max_evaluations, train_iters, num_cores):
    # 加载预训练模型
    pretrain_model_path = os.path.join(root_dir, "AAA_datasets", 'distilled_controllers', load_name, "imitation_model.pt")   #加载预训练后的通用控制模型
    imitation_model = ImitationNet()
    imitation_model.load_state_dict(torch.load(pretrain_model_path,map_location=device))
    ...
    while True:
        ...
        with open(temp_path, "w") as f:
            out = ""
            for structure in structures:
                out += f"{structure.label}\t\t{structure.pretrain_fitness}\t\t{structure.fitness}\n"
            f.write(out)
        ...
        while num_children < (pop_size - num_survivors) and num_evaluations < max_evaluations:
            ...
            if child != None and hashable(child[0]) not in population_structure_hashes:
                ...
                structures[num_survivors + num_children] = ExtendedStructure(*child, num_survivors + num_children)
