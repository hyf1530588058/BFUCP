...
class ActorCritic(nn.Module):
    def __init__(self,):
        ...
        # self.log_std = nn.Parameter(
        #     log_std * torch.ones(1, self.num_actions), requires_grad=True,
        # )
    ...
    def forward(self, structure,inputs,act=None, return_attention=False):  #输入input形状为(4,202)或(128,202)#
        ...
        # _, _, obs_env = self.pre_deal_with_walker_v0(inputs)     #获得全局信息obs_env，obs没有被使用，检查发现obs和obs_mask有点不对劲#
        obs_env = inputs[:, :2]
...
class Agent:
    ...
    @torch.no_grad()
    def act_test(self, structure,inputs):
