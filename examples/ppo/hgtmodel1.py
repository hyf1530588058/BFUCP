...
class GNN(nn.Module):
    ...
    def forward(self, node_feature, WW,mask_matrix,R,node_type,change,rechange):
        ...
        for t_id in range(5):
            ...
            res[idx] = torch.tanh(self.adapt_ws[t_id](node_feature[idx]))   #对不同种类节点特征采取不同的线性映射，将节点特征维度从8变为128，随后输入HGT当中#
        ...
        for gc in self.gcs: 
            meta_xs, attention_map = gc(meta_xs,WW,mask_matrix,R)
