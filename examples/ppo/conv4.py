...
class HGTConv(nn.Module):
    ...
    def forward(self, node_inp, WW,mask_matrix,R,attn_mask):
        ...
        A=torch.matmul(Q.permute(0,1,3,2),K)/self.sqrt_dk #计算key,query后计算注意力矩阵(batch_size,h,n,n)，计算出的结果出现了均分#
        adjusted_A = R.unsqueeze(0).unsqueeze(1) + A  

        # ---- 稳定性改进 ----
        valid_mask = (adjusted_A != float('-inf')).any(dim=-1, keepdim=True).squeeze(0).squeeze(0)
        
        max_vals = torch.where(
            valid_mask,
            adjusted_A.max(dim=-1, keepdim=True).values,
            torch.zeros_like(adjusted_A)
        )
        stabilized_A = adjusted_A - max_vals
        
        exp_A = torch.exp(stabilized_A)
        exp_A = torch.where(valid_mask, exp_A, torch.zeros_like(exp_A))
        
        sum_exp = exp_A.sum(dim=-1, keepdim=True)
        sum_exp = torch.where(sum_exp == 0, 1.0, sum_exp)
        
        attention_weights = exp_A / sum_exp
        attention_weights = attention_weights.squeeze(0).squeeze(0)

        # attention_weights = F.softmax(R+A, dim=3)
        
        attention_weight = torch.where(torch.isnan(attention_weights),torch.full_like(attention_weights,0),attention_weights)
        attention_weights = attention_weight.repeat(1,1,1,20)*attn_mask
        ...
        # has_nan = torch.isnan(self.WK).any().item()
        # print(has_nan)
        ...
        attention_weights = attention_weights[0,0,:,:]  #单独注意力头和batch去看看#
        output = B+node_inp  #残差连接#
