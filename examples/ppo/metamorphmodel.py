...
class TransformerModel(nn.Module):
    def __init__(self, obs_shape = None, decoder_out_dim=1):
        ...
        #嵌入层#
        self.seq_len = 5*5  # 在所有环境中肢体的最大数量#

        #每个肢体的嵌入#
        self.d_model = 128
        self.feature_size = 8
        self.limb_embed = nn.Linear(self.feature_size,self.d_model)   
    ...
    def forward(self, structure,obs_env, local_obs, obs_cm_mask=None, return_attention=False):
        ...
        obs_embed = self.limb_embed(local_obs) * math.sqrt(self.d_model)

        hfield_obs = self.hfield_encoder(obs_env)   #(batch_size,2),将环境单独使用MLP提取#    
        env = torch.unsqueeze(hfield_obs, dim=1)
        env = env.repeat(1, num_nodes, 1).to(device)
        env = env.permute(1,0,2)
        obs_embed = self.pos_embedding(obs_embed)    #添加位置编码#
        obs_embed_t, attention_maps = self.transformer_encoder.get_attention_maps(obs_embed)
        decoder_input = obs_embed_t
        decoder_input = torch.cat([decoder_input, env], axis=2)   #局部观测经transformer后再与环境结合#
...
class ImitationNet(nn.Module):
    ...
    def forward(self, structure, inputs, obs_cm_mask=None, return_attention=False):  
        ...
        change_body = torch.Tensor(list(range(25)))  #构建按节点种类数重新排放顺序的前后映射关系#
        a = b = c = d = e = 0
        length = []
        for node_idx in range(num_nodes):
            feature_list = local_obs[node_idx].tolist()
            # 根据节点类型（0、1、2、3、4）将特征添加到相应的列表中
        ...
        for i in range(25):
            if structure_body_f[i] == 0.0:
                ...
                e = e+1
        rechange = [None for _ in range(25)]   #根据映射关系构建逆映射#
        ...
        N=15
        W = torch.zeros(5, 25, N)   #用于按照节点种类划分节点的W矩阵#
        ...
        R = torch.full((25,25), float('-inf'))      #描述邻居关系的R矩阵，用于跟attention矩阵逐元素相加从而softmax#
        ...
        R_change = R_change[:,change_body.tolist()]  #按照映射关系重新排列R矩阵#
        ...
        obs_rep, attention_maps = self.gnn.forward(node_feature.to(device), WW,mask_matrix,R_change,attn_mask.to(device),node_type,change_body,rechange)  #输入HGT模型，返回张量形状为(feature_dim,batch_size,25)#
        obs_rep = obs_rep[:,:,rechange]   #将节点特征顺序重新排回对应形态顺序#
        ...
        for i in range(len(attention_maps)):
            ...
            attn = attn_list[0]+attn_list[1]+attn_list[2]+attn_list[3]
            attention_maps[i] = attn    
        if obs_env is not None:    #拼接速度（全局信息）#
    ...
    # def forward(self, structure,inputs,  return_attention=False):
    #     local_obs = inputs[:, 6:].reshape(inputs.shape[0], 8, 5 ** 2).permute(0, 2, 1)    #局部信息#
    #     obs_env = inputs[:, :6]
    #     # _, _, obs_env = self.pre_deal_with_walker_v0(inputs)     

    #     pred_act, _ = self.transformer(structure, obs_env, local_obs)
    #     return pred_act
...
class ActorCritic(nn.Module):
    ...
    def forward(self, structure,inputs,act=None, return_attention=False):  #输入input形状为(4,202)或(128,202)#
        
        local_obs = inputs[:, 2:].reshape(inputs.shape[0], 8, 5 ** 2).permute(0, 2, 1)    #局部信息#
        # _, _, obs_env = self.pre_deal_with_walker_v0(inputs)  
        obs_env = inputs[:, :2]   
        obs_mask = torch.tensor(structure[0].flatten()==0).unsqueeze(0).repeat(inputs.shape[0],1).to(device)
        limb_vals, v_attention_maps = self.v_net(       #每个体素的值#
        ...
        val = torch.divide(torch.sum(limb_vals, dim=1, keepdim=True), num_limbs)  #求平均#

        mu, mu_attention_maps = self.mu_net(        #输入decoder获得动作分布的均值#
            structure, obs_env, local_obs
        )
