...
def run_metappo(          #此文件功能与ppo原文件夹中的run文件功能一致#
    ...
    if args.algo == 'a2c':      #判断训练方法从而选择对应的命令行参数#
    ...
    for j in range(num_updates):
        # mdp_sequence = {'states':[],'actions':[],'rewards':[],'next_states':[],'dones':[]}
        ...
        for step in range(args.num_steps):
            # Sample actions
            #print("rollouts.obs[step]:",rollouts.obs[step].shape)
            with torch.no_grad():
                #print("rollouts.obs[step]:",rollouts.obs[step].shape)
                value, action, action_log_prob, recurrent_hidden_states,v_attention_maps = actor_critic.act(     #动作#
                    structure,rollouts.obs[step],envs.action_space, rollouts.recurrent_hidden_states[step],
                    rollouts.masks[step])
                
            noise = torch.from_numpy(0.2 * np.random.normal(size=action.shape)).to(device)
            action = action + noise
            # action = action.cpu().numpy()
            # noisy_stddev = 0.1
            # action_noise = np.random.normal(0,noisy_stddev,action.shape)    
            # noisy_action = np.clip(action + action_noise,envs.action_space.low,envs.action_space.high)
            # action = torch.tensor(noisy_action,device=device)
            ...
            # mdp_sequence['states'].append(rollouts.obs[step].cpu().numpy())
            # mdp_sequence['actions'].append(action.cpu().numpy())
            # mdp_sequence['rewards'].append(reward.cpu().numpy())
            # mdp_sequence['next_states'].append(obs.cpu().numpy())
            # mdp_sequence['dones'].append(done)
        ...
        with torch.no_grad():
            next_value = actor_critic.get_value(
                structure,rollouts.obs[-1], rollouts.recurrent_hidden_states[-1],
                rollouts.masks[-1]).detach()
        # save_dir = os.path.join(origin_dir,"metamorph_carrier",str(saving_convention[1]))  
        # if (j+1)%250 == 0:
        #     torch.save(actor_critic.state_dict(),os.path.join(save_dir,f'model_{j+1}.pt'))  
        #     torch.save(mdp_sequence,os.path.join(save_dir,f'mdp_sequence_{j+1}.pt'))             
        ...
        if j == 0:   #记录zero-shots#
            ...
            determ_avg_reward0 = evaluate(args.num_evals, actor_critic, obs_rms, args.env_name, structure, envs.action_space,args.seed,
                    args.num_processes, eval_log_dir, device)
            file_path = os.path.join(origin_dir,"test_norml_10",str(saving_convention[1]))  
            os.makedirs(file_path, exist_ok=True)
            f = os.path.join(file_path,"step_mean_reward.txt")
            with open(f, 'a') as file:
                file.write(str(j) + "\t\t" + str(determ_avg_reward0) + "\n")         
            # f = os.path.join(file_path,"step_meta_att.txt")
            # with open(f, 'a') as file:
            #     file.write(str(j) + "\t\t" +"V-att:"+ str(v_attention_maps) + "\n")  
        ...
        if (args.eval_interval is not None and len(episode_rewards) > 1
            ...
            obs_rms = utils.get_vec_normalize(envs).obs_rms
            determ_avg_reward = evaluate(args.num_evals, actor_critic, obs_rms, args.env_name, structure, envs.action_space,args.seed,
                    args.num_processes, eval_log_dir, device)
            
            #此处用以记录每个50轮训练后的机器人表现得分#
            file_path = os.path.join(origin_dir,"test_norml_10",str(saving_convention[1]))  
            os.makedirs(file_path, exist_ok=True)
            f = os.path.join(file_path,"step_mean_reward.txt")
            with open(f, 'a') as file:
                file.write(str(j) + "\t\t" + str(determ_avg_reward) + "\n")
            # f = os.path.join(file_path,"step_meta_att.txt")
            # with open(f, 'a') as file:
            #     file.write(str(j) + "\t\t" +"V-att:"+ str(v_attention_maps) + "\n")          
        ...
        if not termination_condition == None:
            if termination_condition(j):
                # file_path = os.path.join(origin_dir,"metamorph-seed11-16p",str(saving_convention[1]))  
                # os.makedirs(file_path, exist_ok=True)
