...
from ppo.myPPOmodel1 import Policy
...
def run_ppo1(          #此文件功能与ppo原文件夹中的run文件功能一致#
    ...
    verbose = True):
    print("-=-=-=-=-=-===========", id(actor_critic),str(saving_convention[1]))
    ...
    for j in range(num_updates):
        ...
        if j == 0:   #记录zero-shots#
            ...
            file_path = os.path.join(origin_dir,"62",str(saving_convention[1]))  
            os.makedirs(file_path, exist_ok=True)
            f = os.path.join(file_path,"step_mean_reward.txt")
            with open(f, 'a') as file:
                file.write(str(j) + "\t\t" + str(determ_avg_reward0) + "\n")         
            f = os.path.join(file_path,"step_HGT_att.txt")
            with open(f, 'a') as file:
