...
import datetime
from .train_adversary import run_TRPO
...
def run_meta(experiment_name, structure_shape, max_evaluations, train_iters, num_cores):   #此文件与GA文件夹中run文件功能一致#
    ...
    while True:
        ...
        for structure in structures:           
            ppo_args = ((structure.body, structure.connections), tc, (save_path_controller, structure.label),generation)  #用于传入多进程并行模块的参数，包括机器人结构和标签，终止条件，控制器保存路径,cichu#
            group.add_job(run_TRPO, ppo_args, callback=structure.set_reward)   #对随机生成的机器人添加训练的进程，可以用于获得奖励#
