...
class TransformerModel(nn.Module):
    ...
    def forward(self, structure, obs_env, local_obs, obs_cm_mask=None, return_attention=False):  
        ...
        WW = WW.repeat(batch_size, 1, 1, 1).to(device)
        ...
        # R = torch.zeros(25,25)
        attn_mask =  torch.zeros(25,20*25)       #(n,20n)#
        for i in range(5):
            for j in range(5):
                if structure_body[i][j]!=0:  
                    a = structure_body[i][j]
                    if i>0:
                        b = structure_body[i-1][j]
                        k = int((a-1)*5+b)
                        attn_mask[(i-1)*5+j][k*25+i*5+j] = 1
                    if i<4:
                        b = structure_body[i+1][j]
                        k = int((a-1)*5+b)
                        attn_mask[(i+1)*5+j][k*25+i*5+j] = 1
                    if j>0:    
                        b = structure_body[i][j-1]
                        k = int((a-1)*5+b)
                        attn_mask[i*5+j-1][k*25+i*5+j] = 1                
                    if j<4:
                        b = structure_body[i][j+1]
                        k = int((a-1)*5+b)
                        attn_mask[i*5+j+1][k*25+i*5+j] = 1 
        attn_mask = attn_mask[change_body.tolist()]
        for i in range(20):
            x = attn_mask[:,i*25:(i+1)*25]
            attn_mask[:,i*25:(i+1)*25] = x[:,change_body.tolist()]       
        ...
        R_change = R_change.unsqueeze(1).expand(-1, 2, -1, -1) .to(device)    #这里是为了保证与A矩阵形状一致，复制2次是因为注意力头数为2            
        ...
        for i in range(len(attention_maps)):
            ...
            attention = torch.zeros(25,25).to(device)
            for j in range(20):
                tensor = attention_map[:,j*25:(j+1)*25]
                attention = torch.add(attention,tensor[:, rechange])
