...
class HGTConv(nn.Module):
    ...
    def forward(self, node_inp, WW,mask_matrix,R):
        ...
        A=torch.matmul(Q.permute(0,1,3,2),K)/self.sqrt_dk #计算key,query后计算注意力矩阵(batch_size,h,n,n)#
        attention_weights = F.softmax(A, dim=3)  #现在是全连接#
        ...
        B=torch.bmm(attention_weights.reshape(batch_size * self.n_heads, n, n),torch.transpose(res_msg.reshape(batch_size * self.n_heads, self.d_k, n),dim0=1, dim1=2)).permute(0,2,1) 
        ...
        attention_weights = attention_weights[0,0,:,:]  #单独注意力头和batch去看看(n,n)#
...
class GeneralConv(nn.Module):
    ...
    def forward(self, meta_xs, WW,mask_matrix,R):
        return self.base_conv(meta_xs,WW,mask_matrix,R)