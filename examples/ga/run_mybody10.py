...
from ppo.mybodymodel10 import num_params
...
def run_mybody(experiment_name, structure_shape, max_evaluations, train_iters, num_cores):   #此文件与GA文件夹中run文件功能一致#
    print()
    ...
    if not is_continuing:    #在新数据中#
        for i in range (max_evaluations):
            # j = i+12
            save_path_structure = os.path.join(root_dir,"robot_universal/upstepper",str(i) + ".npz")
    ...
    while True:
        ...
        for structure in structures:           
            ppo_args = ((structure.body, structure.connections), tc, (save_path_controller, structure.label),actor_critic,experiment_name,args)  #用于传入多进程并行模块的参数，包括机器人结构和标签，终止条件，控制器保存路径,cichu#
            group.add_job(run_mybodyppo10, ppo_args, callback=structure.set_reward)   #对随机生成的机器人添加训练的进程，可以用于获得奖励#
        ...
        f = open(temp_path, "a")
        first_line = "label"+ "\t\t" + "fitness" +"\t\t" +"Ordinate variance" +"\t\t" +"energy" +"\t\t" +"reward1" +"\t\t" +"reward2" +"\t\t" +"reward3"+ "\n"
        out = ""
        for structure in structures:
