...
from ppo.mybodyPPOmodel11 import Policy
...
def run_mybodyppo11(          #此文件功能与ppo原文件夹中的run文件功能一致#
    ...
    for j in range(num_updates):
        ...
        if j == 0:   #记录zero-shots#
            ...
            file_path = os.path.join(origin_dir,"205.5",str(saving_convention[1]))  
        ...
        if not termination_condition == None:
            if termination_condition(j):
                # file_path = os.path.join(origin_dir,"zero-body-10.1",str(saving_convention[1]))  
                # os.makedirs(file_path, exist_ok=True)
