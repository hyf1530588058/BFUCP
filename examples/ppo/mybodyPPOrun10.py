...
from ppo.mybodyPPOmodel10 import Policy
...
def run_mybodyppo10(          #此文件功能与ppo原文件夹中的run文件功能一致#
    ...
    for j in range(num_updates):
        ...
        #     file_path = os.path.join(origin_dir,"test",str(saving_convention[1]))  
        #     os.makedirs(file_path, exist_ok=True)
        #     f = os.path.join(file_path,"step_mean_reward.txt")
        #     with open(f, 'a') as file:
        #         file.write(str(j) + "\t\t" + str(determ_avg_reward0) + "\n")         
        #     f = os.path.join(file_path,"step_mybody_att.txt")
        #     with open(f, 'a') as file:
        ...
        if (args.eval_interval is not None and len(episode_rewards) > 1
            ...
            obs_rms = utils.get_vec_normalize(envs).obs_rms
            determ_avg_reward,reward1,reward2,reward3,variance,action_energy = evaluate(args.num_evals, actor_critic, obs_rms, args.env_name, structure, envs.action_space,args.seed,
                    args.num_processes, eval_log_dir, device)
            ...
            if determ_avg_reward > max_determ_avg_reward:
                ...
                max_action_energy = action_energy
        ...
        if not termination_condition == None:
            if termination_condition(j):
                if verbose:
                    temp_path = os.path.join(root_dir, "saved_data", experiment_name, "output1.txt")
                    f = open(temp_path, "a")
                    line = str(max_reward1)+ "\t\t"+str(max_reward2)+ "\t\t"+str(max_reward3)+ "\n"
                    f.write(line)
                    f.close()                    
                    temp_path = os.path.join(root_dir, "saved_data", experiment_name, "output2.txt")
                    f = open(temp_path, "a")
                    line =  "\t\t"+str(max_variance)+ "\t\t"+str(max_action_energy)+ "\n"
                    f.write(line)
                    f.close()                    
