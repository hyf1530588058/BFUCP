...
def _get_clones(module, N):
    return ModuleList([copy.deepcopy(module) for i in range(N)])   #以列表的形式保存子模块#


def _get_activation_fn(activation):     #激活函数#
    if activation == "relu":
        return F.relu
    elif activation == "gelu":
        return F.gelu

class TransformerEncoder(nn.Module):    #将拿到的encoder_layer通过_get_clones()函数克隆若干份，然后在forward中进行循环的调用#
    __constants__ = ["norm"]

    def __init__(self, encoder_layer, num_layers, norm=None):
        super(TransformerEncoder, self).__init__()
        self.layers = _get_clones(encoder_layer, num_layers)   #将编码器按照规定子编码器层数划分模块#
        self.num_layers = num_layers
        self.norm = norm  

    def forward(self, src, mask=None, src_key_padding_mask=None):   #将embedding和position拼在一起传入encoder#
        output = src

        for l in self.layers:         #遍历所有模块获得输出#
